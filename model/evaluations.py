...
def get_val_pair(path, name):
    ...
    # print(carray.shape, type(carray))
...
def ccrop_batch(imgs):
    ...
    # for img in imgs:
    #     out_image_path = os.path.join("images-for-test", str(image_count) + ".jpg")
    #     cv2.imwrite(out_image_path, cv2.cvtColor(img*255, cv2.COLOR_RGB2GRAY))
    #     # print(np.max(img), np.min(img))
    #     image_count += 1
    resized_imgs = np.array(
        [np.reshape(cv2.resize(cv2.cvtColor(img, cv2.COLOR_RGB2GRAY), (112, 112)), (112, 112, 1)) for img in
         imgs])  # /128
...
def perform_val(embedding_size, batch_size, model,
                carray, issame, nrof_folds=10, is_ccrop=False, is_flip=False):
    """perform val"""
